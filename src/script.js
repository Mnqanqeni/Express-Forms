...
const { validateName, validateAge, validateDate, validateTime, validateComments, validateId } = require("./script_helper");
...
const deleteVisitor = async (visitorId) => {
    ...
    return result.rowCount===0 ? status.visitorNotFound(visitorId) : status.visitorDeleted(visitorId);
};
...
const updateVisitor = async (visitorId, columnKey, newValue) => {
    ...
    return result.rowCount===0 ? status.visitorNotFound(newValue) : status.visitorUpdated(newValue);
};
